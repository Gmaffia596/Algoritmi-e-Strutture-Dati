...
// classe Lista
template< class T >
//List_vector è sottoclasse public di Linear_List con un solo segnaposto (perchè la posizione sarà un int)
...
//sto aggiungendo ai metodi della superclasse, altri metodi
	// costruttori
	List_vector();    
	List_vector(int); 
 ...
 private:
	void change_dimension_(T*& , int , int ); //funzione di array dubling che ha in input un puntatore, che lo farà puntare ad un array di dimensioni K'
	value_type* elements_; //è l'array che allocherò in modo dinamico
	int length_; // the length of the list
	int array_dimension_; // array's dimension
};

// costruttore con dimensione predefinita
template< class T >
 ...
 */
template< class T >
List_vector< T >::List_vector(const List_vector<T>& Lista) { //costruttore di copia
