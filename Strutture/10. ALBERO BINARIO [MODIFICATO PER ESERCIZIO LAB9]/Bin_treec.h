...
class Bin_treec : public Bin_tree<T, int>
{
  ...
  public:
    ...
    struct _cella		//definizione della struttura _CELLA
	{
    	...
    	int numeronodi_sottoalbero; 		//conterrà il numero di nodi del sottoalbero, radice inclusa
    };
    ...
    T read_nodo_sottoalbero(Nodo) const;
    ...
    int conta_nodi_sottoalbero(Nodo);
    void stampa_sottoalbero(Nodo);
...
template <class T>
T Bin_treec<T>::read_nodo_sottoalbero(Nodo n) const		//LETTURA DEL NUMERO DI SOTTOALBERI
{
  if (n != NIL)		//se il nodo n non è vuoto
    return (spazio[n].numeronodi_sottoalbero);	//ritorno il campo dato di n
	else
		throw NullNode();		//altrimenti gestisco l'eccezione di nodo nullo
}
...
template <class T>
int Bin_treec<T>::conta_nodi_sottoalbero(Nodo n)
{	
	if(n!=NIL)
	{
		spazio[n].numeronodi_sottoalbero=1;
		if(!sx_empty(n))
		 {
		 	spazio[n].numeronodi_sottoalbero+=conta_nodi_sottoalbero(spazio[n].sinistro);
		 }
		if(!dx_empty(n))
		{
			spazio[n].numeronodi_sottoalbero+=conta_nodi_sottoalbero(spazio[n].destro);
		}
	}
	return spazio[n].numeronodi_sottoalbero;
}

template <class T>
void Bin_treec<T>::stampa_sottoalbero(Nodo n)
{
	/**std::cout<<spazio[n].valore<<" ";
	//spazio[n].numeronodi_sottoalbero=50;
	std::cout<<conta_nodi_sottoalbero(n);
	std::cout<<spazio[n].numeronodi_sottoalbero;
	n=spazio[n].sinistro;
	std::cout<<spazio[n].valore;**/
	std::cout<<"[";
		if(n!=NIL)
	{
		std::cout<<spazio[n].valore<<" "<<"NS: "<<conta_nodi_sottoalbero(n)<<", ";
		if(!sx_empty(n))
		{
			stampa_sottoalbero(spazio[n].sinistro);
		}
		else std::cout << "NIL";
		std::cout << ", ";
		if(!dx_empty(n))
		{
			stampa_sottoalbero(spazio[n].destro);
		}
...
template <class T>
void Bin_treec<T>::previsit(Nodo n)
{
	if(n != NIL)
	{
		std::cout<<spazio[n].valore<<" ";
		if(!sx_empty(n))
		{
			previsit(spazio[n].sinistro);
		}
		
		if(!dx_empty(n))
		{
			previsit(spazio[n].destro);
		}
...
template <class T>
void Bin_treec<T>::invisita(Nodo n)
{
	if(n != NIL)
	{
		if(!sx_empty(n))
...
template <class T>
void Bin_treec<T>::postvisit(Nodo n)
{
	if(n != NIL)
	{
		if(!sx_empty(n))
		{
			postvisit(spazio[n].sinistro);
		}
		if(!dx_empty(n))
		{
			postvisit(spazio[n].destro);
		}
		std::cout<<spazio[n].valore<<" ";
	}
	else throw NullNode();
