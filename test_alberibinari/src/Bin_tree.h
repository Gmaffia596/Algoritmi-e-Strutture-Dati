...
class Bin_tree {
  ...
  public:
	...
	virtual int height(Nodo) const = 0;
...
void Bin_tree<T,N>::print() const{
	if (!empty())
		printSubTree(root());     //stampa sottoalbero
	else
		std::cout << "[]" << std::endl;
	std::cout << std::endl;
}
...
//stampa il sottoalbero partendo dal nodo n in input
template <class T, class N>
void Bin_tree<T,N>::printSubTree(const Nodo n) const{
	std::cout << "[" << read(n) << ", ";
	if (!sx_empty(n))
		printSubTree(sx(n));      //stampa sottoalbero sx (se non vuoto)
	else std::cout << "NIL";
	std::cout << ", ";
	if (!dx_empty(n))
		printSubTree(dx(n));      //stampa sottoalbero dx (se non vuoto)
	else std::cout << "NIL";
	std::cout << " ]";
}
